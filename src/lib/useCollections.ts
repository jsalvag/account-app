...
export function useUserCollections(userId?: string | null) {
  ...
  useEffect(() => {
    ...
    const unsubInst = onSnapshot(qInst, s => setInstitutions(s.docs.map(d => ({id: d.id, ...(d.data() as Omit<Institution, 'id'>)}))), e => setErr(`institutions: ${e.message || e}`));
    const unsubAcc = onSnapshot(qAcc, s => setAccounts(s.docs.map(d => ({id: d.id, ...(d.data() as Omit<Account, 'id'>)}))), e => setErr(`accounts: ${e.message || e}`));
...
export async function transfer(userId: string, fromId: string, toId: string, amount: number) {
  await runTransaction(db, async (tx) => {
    ...
    const f = fs.data() as Account;
    const t = ts.data() as Account;

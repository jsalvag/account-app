...
export default function Money() {
  ...
  const toggleTransfer = (): void => setShowTransfer(v => !v);
  const toggleFx = (): void => setShowFx(v => !v);
  const toggleIncome = (): void => setShowIncome(v => !v);

  const accOptions = useMemo(() => accounts.map(a => {
    const inst = institutions.find(i => i.id === a.institutionId);
    return {
      id: a.id,
      label: `${inst?.name ?? "(??)"} — ${a.name} [${a.currency}] — ${money(a.balance)}`,
      currency: a.currency
    };
  ...
  async function onCreateInstitution(e: React.FormEvent) {
    ...
    try {
      await createInstitution(uid, instName.trim(), instKind);
      setInstName("");
    } catch (e: any) {
  ...
  async function onCreateAccount(e: React.FormEvent) {
    ...
    try {
      await createAccount(uid, acctInst, acctName.trim(), acctCurr, isFinite(init) ? init : 0);
      setAcctName("");
      setAcctInit("");
    } catch (e: any) {
      setErr?.(e.message || String(e));
  ...
  async function onTransfer(e: React.FormEvent) {
    e.preventDefault();
    const amt = Number(xAmt || 0);
    if (!xFrom || !xTo || !(amt > 0)) return;
    try {
      await transfer(uid, xFrom, xTo, amt);
      setXFrom("");
      setXTo("");
      setXAmt("");
    } catch (e: any) {
  ...
  async function onFx(e: React.FormEvent) {
    ...
    try {
      await fx(uid, fFrom, fTo, amt, rate);
      setFFrom("");
      setFTo("");
      setFAmt("");
      setFRate("");
  ...
  const onIncome = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
    const amt = typeof incAmt === "string" ? Number(incAmt || 0) : incAmt;
    if (!incAcct || !(amt > 0)) return;
    try {
      await income(uid, incAcct, amt, incNote.trim() || undefined);
      setIncAcct("");
      setIncAmt("");
      setIncNote("");
    } catch (err) {
  ...
  return (
    <div className="grid gap-6">
      {/* ACTIONS TOOLBAR + COLLAPSIBLE SECTIONS */}
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700 ${showTransfer ? 'bg-slate-100 dark:bg-slate-800' : ''}`}
          ...
          <button
            type="button"
            onClick={toggleFx}
            className={`rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700 ${showFx ? 'bg-slate-100 dark:bg-slate-800' : ''}`}
          ...
          <button
            ...
            className={`rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700 ${showIncome ? 'bg-slate-100 dark:bg-slate-800' : ''}`}
      ...
      {showTransfer && (
        <div id="transfer-panel"
          ...
          <form onSubmit={onTransfer} className="grid gap-2">
            ...
            <select value={xTo} onChange={e => setXTo(e.target.value)} disabled={!xFrom}
                    className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800">
              <option
                value="">{xFrom ? 'Selecciona cuenta destino…' : 'Selecciona origen primero…'}</option>
              {xToOptions.map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
            </select>
            <input value={xAmt} onChange={e => setXAmt(e.target.value as unknown as number | '')}
                   type="number" step="0.01" placeholder="Monto"
                   className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
            <div className="flex items-center gap-2">
              <button
                className="rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700">Transferir
              </button>
              <button type="button" onClick={toggleTransfer}
                      className="text-sm opacity-80 hover:opacity-100">Cerrar
              </button>
            </div>
          </form>
        </div>
      ...
      {showFx && (
        <div id="fx-panel"
          ...
          <form onSubmit={onFx} className="grid gap-2">
            ...
            <input value={fAmt} onChange={e => setFAmt(e.target.value as unknown as number | '')}
                   type="number" step="0.01" placeholder="Monto a vender"
                   className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
            <select value={fTo} onChange={e => setFTo(e.target.value)} disabled={!fFrom}
                    className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800">
              <option
                value="">{fFrom ? 'Selecciona cuenta destino…' : 'Selecciona origen primero…'}</option>
              {fToOptions.map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
            </select>
            <input value={fRate} onChange={e => setFRate(e.target.value as unknown as number | '')}
                   type="number" step="0.0001" placeholder="Tasa (to/from)"
                   className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
            <div className="flex items-center gap-2">
              <button
                className="rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700">Convertir
              </button>
      ...
      {showIncome && (
        <div id="income-panel"
             className="rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
          <h3 className="text-lg font-semibold mb-3">Ingreso de dinero</h3>
          <form onSubmit={onIncome} className="grid gap-2">
            <select value={incAcct} onChange={(e) => setIncAcct(e.target.value)}
                    className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800">
              <option value="">Selecciona cuenta destino…</option>
              {accOptions.map((o) => (<option key={o.id} value={o.id}>{o.label}</option>))}
            </select>
            <input value={incAmt}
                   onChange={(e) => setIncAmt(e.target.value as unknown as number | '')}
                   type="number" step="0.01" placeholder="Monto a ingresar"
                   className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
            ...
            <div className="flex items-center gap-2">
              <button
                className="rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700">Agregar
                ingreso
              </button>
              <button type="button" onClick={toggleIncome}
                      className="text-sm opacity-80 hover:opacity-100">Cerrar
              </button>
            </div>
          </form>
      ...
      <div className="rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
        ...
        <form onSubmit={onCreateInstitution} className="grid gap-2 md:grid-cols-3 mb-3">
          <input value={instName} onChange={e => setInstName(e.target.value)}
                 ...
                 className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800 md:col-span-2"/>
          <select value={instKind} onChange={e => setInstKind(e.target.value as any)}
                  className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800">
          ...
          <button
            className="md:col-span-3 rounded-md px-3 py-2 border border-slate-300 dark:border-slate-700">Agregar
            entidad
          </button>
        </form>
              ...
              <tr key={i.id} className="border-t border-slate-200/60 dark:border-slate-800/60">
                ...
                <td className="py-2 pl-4 text-right">
                  <button onClick={async () => {
                    const nn = prompt("Nuevo nombre de la entidad:", i.name || "");
                    if (nn === null) return;
                    const kinds = ["bank_physical", "bank_virtual", "wallet", "broker", "crypto_exchange", "cash"] as const;
                    const nk = prompt("Tipo (bank_physical, bank_virtual, wallet, broker, crypto_exchange, cash):", i.kind) as any;
                    if (nk === null || !kinds.includes(nk)) return;
                    await updateInstitution(i.id, {name: String(nn).trim(), kind: nk});
                  }} className="text-xs rounded-md px-2 py-1 border mr-2">Editar
                  </button>
                  <button onClick={async () => {
                    if (!confirm(`¿Eliminar entidad "${i.name}" y sus cuentas?`)) return;
                    await deleteInstitutionCascade(uid, i.id);
                  }} className="text-xs rounded-md px-2 py-1 border">Borrar
                  </button>
                </td>
              </tr>
            ))}
            {institutions.length === 0 && <tr>
                <td colSpan={3} className="py-4 text-center opacity-60">Sin entidades aún</td>
            </tr>}
            </tbody>
          </table>
        </div>
        ...
        <form onSubmit={onCreateAccount} className="grid gap-2 md:grid-cols-5 mb-3">
          <select value={acctInst} onChange={e => setAcctInst(e.target.value)}
          ...
          <input value={acctName} onChange={e => setAcctName(e.target.value)}
                 ...
                 className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
          <select value={acctCurr} onChange={e => setAcctCurr(e.target.value)}
                  className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800">
            <option>ARS</option>
            <option>USD</option>
            <option>EUR</option>
          ...
          <input value={acctInit} onChange={e => setAcctInit(e.target.value as any)} type="number"
                 step="0.01" placeholder="Saldo inicial (opcional)"
                 className="rounded-md border px-3 py-2 bg-white dark:bg-slate-800"/>
          <button
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full w-full text-sm">
            ...
            {accounts.map(a => {
              ...
              return (
                <tr key={a.id} className="border-t border-slate-200/60 dark:border-slate-800/60">
                  ...
                  <td className="py-2 pl-4 text-right">
                    <button onClick={async () => {
                      const nn = prompt("Nuevo nombre de cuenta:", a.name || "");
                      if (nn === null) return;
            ...
            {accounts.length === 0 && <tr>
                <td colSpan={5} className="py-4 text-center opacity-60">Sin cuentas aún</td>
            </tr>}
            </tbody>
          </table>
        </div>
